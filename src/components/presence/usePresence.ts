...
import { isPokemonId } from "./pokedex";
...
function readAvatar() {
  try {
    ...
    // Re-roll rather than trust the stored id: anything on the dex is a
    // valid choice now, but a number that isn't one has no sprite.
    if (isPokemonId(n)) return n;
